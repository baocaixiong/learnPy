...
class TalkOnQzone(object):
    ...
    def GetVerifyCode(self, uin):
        check = self.Get("http://check.ptlogin2.qq.com/check?regmaster=&uin=%s&appid=%s&r=%s"%(uin, self.appid, random.Random().random()), self.DefaultHeaders)
        verify =  eval(check.split("(")[1].split(")")[0])
        verify = list(verify)
        if verify[0] == "1":
            img = "http://captcha.qq.com/getimage?uin=%s&aid=%s&%s"%(uin, self.appid, random.Random().random())
            with open("verify.jpg","wb") as f:
                rr = urllib2.Request(url=img, headers=self.DefaultHeaders)
                f.write(urllib2.urlopen(rr).read())
            os.popen("./verify.jpg")
            verify[1] = raw_input("需要输入验证码，请输入打开的图片\"verify.jpg\"中的验证码：\n").strip()
        return verify
